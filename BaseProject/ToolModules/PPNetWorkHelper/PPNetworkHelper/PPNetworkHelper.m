...
+ (NSURLSessionTask *)GET:(NSString *)URL
    ...
    NSURLSessionTask *sessionTask = [_sessionManager GET:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        //配置成功请求的回调
        NSString *status = [responseObject[@"code"] stringValue];
        BOOL isSuccess = [status isEqualToString:@"1"];
        NSString *hint = responseObject[@"msg"];
        NSDictionary *data = responseObject[@"data"];
        success ? success(isSuccess,status,hint,data,responseObject) : nil;
        //对数据进行异步缓存
...
#pragma mark - 上传文件
+ (NSURLSessionTask *)uploadFileWithURL:(NSString *)URL  parameters:(id)parameters name:(NSString *)name filePath:(NSString *)filePath progress:(PPHttpProgress)progress  success:(PPHttpRequestSuccess)success  failure:(PPHttpRequestFailed)failure {
...
name:(NSString *)name  images:(NSArray<UIImage *> *)images  fileNames:(NSArray<NSString *> *)fileNames    imageScale:(CGFloat)imageScale  imageType:(NSString *)imageType
 progress:(PPHttpProgress)progress success:(PPHttpRequestSuccess)success
 failure:(PPHttpRequestFailed)failure {
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
